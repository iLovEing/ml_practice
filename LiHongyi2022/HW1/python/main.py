...
def select_feat(train_data, valid_data, test_data, select_all=True):
    # TODO: Selects useful features to perform regression
...
def trainer(train_loader, valid_loader, model, config, device):
    ...
    for epoch in range(n_epochs):
        ...
        mean_train_loss = sum(loss_per_epoch) / len(loss_per_epoch)
        ...
        mean_valid_loss = sum(loss_per_epoch) / len(loss_per_epoch)
...
def save_pred(preds, file):
    """Save predictions to specified fil"""
...
def plot_loss_curve(train_loss, valid_loss):
    ...
    for it in range(len(train_loss)):
        data.append({
            "epoch": it,
            "loss": train_loss[it],
            "type": "train_loss",
...
if __name__ == "__main__":
    ...
    config = {
        'seed': 19961111,      # Your seed number, you can pick your lucky number. :)
